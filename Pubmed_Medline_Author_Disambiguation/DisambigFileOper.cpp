...
bool fetch_records_from_sqlite3(list <cRecord> & source, const char * sql_file, const char * statement, const map<string, asgdetail>& asgtree){
	...
	for ( unsigned int i = 0; i < num_cols; ++i ) {
        ...
        /////// PUBMED CHANGE //////////
        else if ( cRecord::column_names[i] == cSuffix::class_name) {
            cSuffix::enable();
            pointer_array[i] = new cSuffix;
            cSuffix::column_index_in_query = i;
        }
        //////////////////////////////
        ...
        ////// PUBMED CHANGE  /////
        // add article attribute classes
        else if (cRecord::column_names[i] == cTitleWords::class_name){
            cTitleWords::enable();
            pointer_array[i] = new cTitleWords();
            cTitleWords::column_index_in_query = i;
        }
        else if (cRecord::column_names[i] == cJournal::class_name){
            cJournal::enable();
            pointer_array[i] = new cJournal();
            cJournal::column_index_in_query = i;
        }
        else if (cRecord::column_names[i] == cMeSH::class_name){
            cMeSH::enable();
            pointer_array[i] = new cMeSH();
            cMeSH::column_index_in_query = i;
        }
        else if ( cRecord::column_names[i] == cAffiliations::class_name){
            cAffiliations::enable();
            pointer_array[i] = new cAffiliations();
            cAffiliations::column_index_in_query = i;
        }
        else if (cRecord::column_names[i] == cAffiliation_Exists::class_name){
            cAffiliation_Exists::enable();
            pointer_array[i] = new cAffiliation_Exists();
            cAffiliation_Exists::column_index_in_query = i;
        }
        else if ( cRecord::column_names[i] == cLanguage::class_name ) {
            cLanguage::enable();
            pointer_array[i] = new cLanguage();
            cLanguage::column_index_in_query = i;
        }
        //////////////////////////////
		else {
