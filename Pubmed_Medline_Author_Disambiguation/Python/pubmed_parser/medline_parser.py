...
Parsers for MEDLINE XML
 ...
 This file is altered from its original implementation in the pubmed_parser package!
 It extracts necessary fields for Torvik disambiguation  (not all included in
  original implementation) and returns mesh terms as a list
"""
import re
import numpy as np
...
def parse_article_info(
    ...
    if article.find("Language") is not None:
        language = stringify_children(article.find("Language")).strip() or ""
    else:
        language = ""
