...
public:
    ...
    cSuffix(const char * source = NULL ) {}
    unsigned int compare(const cAttribute & rhs) const { return this == & rhs; };        //override to allow customization.
    unsigned int get_attrib_max_value() const {
        if ( ! is_comparator_activated() )
            cAttribute::get_attrib_max_value();
        return max_value;
...
class cPubDate: public cAttribute_Single_Mode<cPubDate> {
public:
...
///// PUBMED CHANGE /////
// Add Pubmed Single Mode Attributes
class cJournal : public cAttribute_Single_Mode <cJournal> {
public:
    static const unsigned int max_value = 1;
    
    cJournal(const char * source = NULL ) {}
...
class cAffiliation_Exists : public cAttribute_Single_Mode <cAffiliation_Exists> {
public:
    static const unsigned int max_value = 1;
...
class cLanguage : public cAttribute_Single_Mode <cLanguage> {
public:
    static const unsigned int max_value = 3;
